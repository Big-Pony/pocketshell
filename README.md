...
- **文件 + Git 面板**：惰性目录树 + 内联 git 标记；只读代码预览（高亮 + 行号）；工作区 diff 可视化；只读 git log/分支/status；文件写操作（重命名/新建/删除强确认）；上传下载（多文件带进度、目录 zip、分片传输）。
...
- **File + Git panel** — lazy tree with inline git markers; read-only code preview (highlight + line numbers); working-tree diff; read-only git log/branches/status; file ops (rename/new/delete with confirm); upload/download (multi-file with progress, dir-as-zip, chunked transfer).
