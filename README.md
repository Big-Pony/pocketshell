...
- **文件 + Git 面板**：惰性目录树 + 内联 git 标记；代码预览（高亮 + 行号），可切到编辑器编辑保存（CodeMirror 6，行号 / 搜索替换 / 系统输入法，分片保存 + 基于 mtime 的防误覆盖，新建文件直接进编辑器）；工作区 diff 可视化；只读 git log/分支/status；文件写操作（重命名/新建/删除强确认）；上传下载（多文件带进度、目录 zip、分片传输）。
...
- **File + Git panel** — lazy tree with inline git markers; code preview (highlight + line numbers) that switches to an editor (CodeMirror 6 — line numbers, find/replace, native IME, chunked save with mtime-based overwrite guard, new files open straight into the editor); working-tree diff; read-only git log/branches/status; file ops (rename/new/delete with confirm); upload/download (multi-file with progress, dir-as-zip, chunked transfer).
