...
- **自定义全键盘**：完整笔记本布局，F1–F12、方向键、修饰键三态 sticky
- **输入法整段发送**：用系统输入法写完一整段需求再一次性注入，断线也不丢缓冲
- **智能命令提示条**：按前缀联想，点一下补全
- **Fn 应用快捷层**：切标签、滚屏、新建会话、全屏、重命名，全部走组合键
...
- **6 套主题**：深色 4 套（石墨橙 / 示波青 / 纯黑银 / 普鲁士蓝）+ 浅色 2 套（暖白仪表 / 朱砂宣纸），可跟随系统明暗
...
当前版本 **v1.8.0**（2026-08-03）· 完整更新日志见 [Releases](https://github.com/Big-Pony/pocketshell/releases)

**v1.8.0** —— 移除网页管理页（存在公网匿名访问的高危问题），设备管理统一走命令行；`devices remove` 对常驻 Agent 即时生效；应用内更新限定官方仓库。

**v1.7.0** —— 分割条显示 AI 上下文用量（四家工具）；Kimi CLI 接入推送通知；视频预览支持拖动进度条；Git 分支行加刷新按钮。

**v1.6.0** —— `install` / `uninstall` 子命令，一条命令装成开机自启的服务；一行安装脚本（校验 SHA256）；修复 `pair` 在服务启动 5 分钟内失效。
