...
/** 把文本切成 ≤chunkBytes 的字节分片。二阶段起走二进制帧，不再 base64。 */
export function encodeWriteChunks(text: string, chunkBytes = CHUNK_BYTES): Uint8Array[] {
  const bytes = new TextEncoder().encode(text);
  if (bytes.length === 0) return [bytes];
  const out: Uint8Array[] = [];
  for (let o = 0; o < bytes.length; o += chunkBytes) out.push(bytes.subarray(o, o + chunkBytes));
  return out;
}
...
export async function saveFile(conn: RpcLike, path: string, text: string, expectMtime?: number): Promise<{ mtime: number }> {
  ...
  for (let i = 0; i < chunks.length; i++) {
    ...
    const params: Record<string, unknown> = { writeId, first: i === 0, last };
    if (last) { params.path = path; if (expectMtime !== undefined) params.expectMtime = expectMtime; }
    const r = (await conn.rpcBin("fs.write", params, chunks[i])) as { ok?: true; mtime?: number };
